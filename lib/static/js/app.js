...
function loadAgents(testId) {
    ...
    // Rename artifact tabs for run_all_test
    const reportTab = document.getElementById('tab-report');
    const specTab = document.getElementById('tab-spec');
    const consTab = document.getElementById('tab-consolidated');

    if (testId === 'run_all_test') {
        reportTab.style.display = 'none';
        consTab.style.display = 'inline-block';
        specTab.style.opacity = '0.45';
        specTab.title = 'No single spec for run_all_test';
    } else {
        reportTab.style.display = 'inline-block';
        consTab.style.display = 'none';
        specTab.style.opacity = '';
        specTab.title = '';
    }
}
...
function markAgentSkipped(name) {
    const card = document.getElementById('card-' + name);
    if (card && !card.classList.contains('failed')) {
        card.className = 'agent-card skipped';
        document.getElementById('status-' + name).textContent = 'Skipped ⏭️';
    }
}
...
function appendLog(line) {
    ...
    // Detect pipeline "XxxAgent skipped" lines
    const skipMatch = line.match(/\[Pipeline\] (\w+) skipped/);
    if (skipMatch && AGENTS.includes(skipMatch[1])) {
        markAgentSkipped(skipMatch[1]);
    }
...
function showTab(type) {
    ...
    // Friendly note for spec tab in run_all_test mode
    if (type === 'spec' && currentTest === 'run_all_test') {
        content.innerHTML =
            '<div class="artifact-empty">&#x1F4CC; No single spec file for <strong>run_all_test</strong> &mdash; view individual test specs from their own entries.</div>';
        return;
    }
    ...
    fetch(`/api/artifact?test=${encodeURIComponent(currentTest)}&type=${type}`)
        ...
        .then(data => {
            if (data.error) {
                content.innerHTML =
                    `<div class="artifact-empty">\u26a0\ufe0f ${data.error}</div>`;
                return;
            }
            if (type === 'spec') {
